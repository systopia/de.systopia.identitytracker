 ...
 * extension.
 */
class CRM_Identitytracker_ExtensionUtil {
  const SHORT_NAME = 'identitytracker';
  const LONG_NAME = 'de.systopia.identitytracker';
  const CLASS_PREFIX = 'CRM_Identitytracker';
 ...
 */
function _identitytracker_civix_civicrm_postInstall() {
  _identitytracker_civix_civicrm_config();
  if ($upgrader = _identitytracker_civix_upgrader()) {
 ...
 */
function _identitytracker_civix_civicrm_themes(&$themes) {
  $files = _identitytracker_civix_glob(__DIR__ . '/*.theme.php');
  foreach ($files as $file) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _identitytracker_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _identitytracker_civix_civicrm_entityTypes(&$entityTypes) {
